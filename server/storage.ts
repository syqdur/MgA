import { users, rootAdmins, galleries, platformUsers, galleryProfiles, type User, type InsertUser, type RootAdmin, type InsertRootAdmin, type Gallery, type InsertGallery, type PlatformUser, type InsertPlatformUser, type GalleryProfile, type InsertGalleryProfile } from "@shared/schema";
import { db } from "./db";
...
export interface IStorage {
  ...
  // Gallery profile methods
  getGalleryProfileByFirebaseId(firebaseId: string): Promise<GalleryProfile | undefined>;
  createGalleryProfile(profile: InsertGalleryProfile): Promise<GalleryProfile>;
  updateGalleryProfile(firebaseId: string, profile: Partial<GalleryProfile>): Promise<GalleryProfile>;
  deleteGalleryProfile(firebaseId: string): Promise<void>;
}
...
export class MemoryStorage implements IStorage {
  ...
  async getGalleryProfileByFirebaseId(firebaseId: string): Promise<GalleryProfile | undefined> {
    return this.galleryProfiles.find(p => p.firebaseId === firebaseId);
  }

  async createGalleryProfile(insertProfile: InsertGalleryProfile): Promise<GalleryProfile> {
    const profile: GalleryProfile = {
      id: this.nextGalleryProfileId++,
      firebaseId: insertProfile.firebaseId,
      name: insertProfile.name,
      bio: insertProfile.bio || null,
      profilePicture: insertProfile.profilePicture || null,
      theme: insertProfile.theme || "hochzeit",
      createdAt: new Date(),
      updatedAt: new Date()
    };
    this.galleryProfiles.push(profile);
    return profile;
  }

  async updateGalleryProfile(firebaseId: string, profileUpdate: Partial<GalleryProfile>): Promise<GalleryProfile> {
    const index = this.galleryProfiles.findIndex(p => p.firebaseId === firebaseId);
    if (index === -1) {
      throw new Error('Gallery profile not found');
    }
    
    const updatedProfile = {
      ...this.galleryProfiles[index],
      ...profileUpdate,
      updatedAt: new Date()
    };
    this.galleryProfiles[index] = updatedProfile;
    return updatedProfile;
  }

  async deleteGalleryProfile(firebaseId: string): Promise<void> {
    const index = this.galleryProfiles.findIndex(p => p.firebaseId === firebaseId);
    if (index === -1) {
      throw new Error('Gallery profile not found');
    }
    this.galleryProfiles.splice(index, 1);
  }
}
...
export class DatabaseStorage implements IStorage {
  ...
  async getGalleryProfileByFirebaseId(firebaseId: string): Promise<GalleryProfile | undefined> {
    const [profile] = await db.select().from(galleryProfiles).where(eq(galleryProfiles.firebaseId, firebaseId));
    return profile || undefined;
  }

  async createGalleryProfile(insertProfile: InsertGalleryProfile): Promise<GalleryProfile> {
    const [profile] = await db
      .insert(galleryProfiles)
  ...
  async updateGalleryProfile(firebaseId: string, profileUpdate: Partial<GalleryProfile>): Promise<GalleryProfile> {
    const [profile] = await db
      .update(galleryProfiles)
      .set({...profileUpdate, updatedAt: new Date()})
      .where(eq(galleryProfiles.firebaseId, firebaseId))
      .returning();
    return profile;
  }

  async deleteGalleryProfile(firebaseId: string): Promise<void> {
    await db.delete(galleryProfiles).where(eq(galleryProfiles.firebaseId, firebaseId));
  }
}
