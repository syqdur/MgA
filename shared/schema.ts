...
// Gallery profiles table for gallery-specific display settings
export const galleryProfiles = pgTable("gallery_profiles", {
  id: serial("id").primaryKey(),
  firebaseId: text("firebase_id").notNull().unique(),
  name: text("name").notNull(),
  bio: text("bio"),
...
export const insertGalleryProfileSchema = createInsertSchema(galleryProfiles).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export type InsertGalleryProfile = z.infer<typeof insertGalleryProfileSchema>;
export type GalleryProfile = typeof galleryProfiles.$inferSelect;
